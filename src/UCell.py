...
signatures = {'UCell_score': gene_indices}

# Run pyUCell scoring
puc.compute_ucell_scores(adata, signatures, layer='log1p', suffix='')

print(f"UCell scores computed for {adata.n_obs} cells")
...
if 'X_umap' in adata.obsm:
    ...
    ax.set_xlabel('UMAP1')
    ax.set_ylabel('UMAP2')
    ax.set_title(f'UCell Score on UMAP ({args.prefix})')
    cbar = plt.colorbar(scat, ax=ax)
    cbar.set_label('UCell Score')
    plt.tight_layout()
    plt.savefig(f'figures/{args.prefix}_umap.png', dpi=150)
    plt.close()

# -------------------------
# Plot 3: Per-sample UMAP
# -------------------------
if 'X_umap' in adata.obsm and 'sample' in adata.obs.columns:
    ...
    for sample in unique_samples:
        ...
        ax.set_xlabel('UMAP1')
        ax.set_ylabel('UMAP2')
        ax.set_title(f'UCell Score - {sample} ({args.prefix})')

        cbar = plt.colorbar(scat, ax=ax)
        cbar.set_label('UCell Score')

        plt.tight_layout()
        plt.savefig(f'figures/{args.prefix}_umap_{sample}.png', dpi=150)
        plt.close()

print(f"All figures saved to figures/{args.prefix}_ucell_*")
